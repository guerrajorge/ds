...
SPARK_URL=http://apache.mirrors.tds.net/spark//spark-2.4.3/spark-2.4.3-bin-hadoop2.7.tgz

# The Spark folder name should be the same as the name of the file being downloaded as specified in the SPARK_URL
SPARK_FOLDER_NAME=spark-2.4.3-bin-hadoop2.7.tgz

# Find the proper md5 hash from the Apache site
SPARK_MD5=5d79926ad35cb3b214ed4f7878063aa5
...
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
    if [[ ! -d $SHELL_PROFILE/scripts ]]
    then
        mkdir $SHELL_PROFILE/scripts
        echo "export PATH=\$PATH:$SHELL_PROFILE/scripts" >> $SHELL_PROFILE
    else
...
then
    ...
    if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        --packages org.apache.hadoop:hadoop-aws:2.7.3" > $SHELL_PROFILE/scripts/jupyspark.sh

        chmod +x $SHELL_PROFILE/scripts/jupyspark.sh
    fi
    ...
    if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        \$1" > $SHELL_PROFILE/scripts/localsparksubmit.sh

        chmod +x $SHELL_PROFILE/scripts/localsparksubmit.sh
    fi
fi
